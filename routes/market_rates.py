...
def fetch_msamb_by_crop(crop_mr):
    """Fetch MSAMB data for a given crop (Marathi name)."""
    driver = webdriver.Chrome()
    try:
        driver.get("https://www.msamb.com/ApmcDetail/APMCPriceInformation")
        wait = WebDriverWait(driver, 10)

        # Wait for the crop dropdown
...
def get_rates_for_city(df, city_en):
    """Filter DataFrame for a city (English name)."""
    city_mr = city_en_to_mr.get(city_en.lower())
    if not city_mr:
        print(f"City '{city_en}' not found in dictionary.")
        return None
...
def main():
    ...
    if not crop_mr:
        print(f"Crop '{crop_en}' not found in dictionary.")
        return

    df = fetch_msamb_by_crop(crop_mr)
    filtered = get_rates_for_city(df, city_en)
    if filtered is not None:
        # Show only relevant columns
        columns_to_show = ['बाजार समिती', 'किमान', 'कमाल', 'City (English)']
        print(filtered[columns_to_show].to_string(index=False))

        # Optional: save to CSV
        save_csv = input("Save results to CSV? (y/n): ").strip().lower()
        if save_csv == 'y':
            filename = f"{crop_en}_{city_en}_rates.csv"
