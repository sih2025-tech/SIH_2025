...
# (Sentinel-2 + Sentinel-1 + DEM + NASA POWER + Merge + Fill NaNs)
# Trains on 100 diverse India locations for best predictions
...
# -------------------------------
def initialize_gee(project_id='weather4-470605'):
    try:
        ee.Initialize(project=project_id)
        print(f"🌍 GEE Initialized with project {project_id}")
    except ee.EEException:
        print("⚠️ GEE not authenticated. Attempting authentication...")
        ee.Authenticate()
        try:
            ee.Initialize(project=project_id)
            print(f"🌍 GEE Initialized with project {project_id} after authentication")
        except ee.EEException:
            print("❌ Failed to initialize GEE. Exiting.")
            sys.exit(1)
...
# -------------------------------
# Step 2: Fetch Sentinel-2 NDVI + bands
# -------------------------------
def fetch_sentinel2(lat, lon, start_date, end_date):
    ...
    # Convert each reduced Image to a Feature
...
# -------------------------------
# Step 4: Fetch DEM from GEE
# -------------------------------
def fetch_dem(lat, lon):
    point = ee.Geometry.Point([lon, lat])
    try:
        dem = ee.Image("USGS/SRTMGL1_003").sample(point, scale=30).first().get('elevation').getInfo()
        return dem
    except:
        print("❌ DEM fetch failed, using fallback 200")
        return 200

# -------------------------------
# Step 5: Fetch NASA POWER Weather Data
# -------------------------------
...
# -------------------------------
# Step 6: Merge Data and Fill NaNs
# -------------------------------
def merge_data(ndvi_df, s1_df, nasa_df, dem):
    all_dates = []
...
# -------------------------------
# Step 7: ML Prediction with Quantile Regression
# -------------------------------
def ml_predict(df):
    import pandas as pd
    import numpy as np
...
# -------------------------------
# Step 8: Reduced India-wide training coordinates (35 diverse locations)
# -------------------------------
...
if __name__ == "__main__":
    ...
    if not os.path.exists(model_file):
        print("Training global model across all India coordinates...")
        all_data = []
        ...
        def fetch_for_location(lat, lon):
            ...
            nasa_df = fetch_nasa_power(lat, lon, start_date, end_date)
            merged_df = merge_data(ndvi_df, s1_df, nasa_df, dem)
            return merged_df

        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = {executor.submit(fetch_for_location, lat, lon): (lat, lon) for lat, lon in maharashtra_coordinates}
        ...
        print("Running ML prediction on all India data...")
    ...
    # -------------------------------
    # Predict for user-input specific location
    # -------------------------------
    ...
    dem = fetch_dem(lat, lon)
    nasa_df = fetch_nasa_power(lat, lon, start_date, end_date)
    final_df = merge_data(ndvi_df, s1_df, nasa_df, dem)

    # ----------------------------
    # 1. Load model + feature names
    # ----------------------------
    stack_model = joblib.load("india_global_model.pkl")
    ...
    def prepare_features(df):
        X = df.drop(columns=['date', 'predicted_rainfall'], errors='ignore')
